...
8. **프리랜서 로스터는 하드코딩(`SHEET_TO_INFO`)** — 번역 요청 대상 목록·대시보드·이모지 워크플로는 `index.html`의 `SHEET_TO_INFO` 맵(`탭이름 → {영문명, 언어}`) 기준. **프리랜서가 늘면 이 맵에 항목을 추가하고 배포해야** 반영됨(`영문명`은 `List` 탭의 이름 칸 값과 일치해야 `g_listInfo`에서 매칭). 계약서 탭만 예외로 `List` 탭 기반.
